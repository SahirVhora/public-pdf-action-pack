...
def split_lines(text: str) -> list[str]:
    return [
        normalise_spaces(line) for line in text.splitlines() if normalise_spaces(line)
    ]
...
def extract_costs_with_context(text: str) -> list[tuple[str, str]]:
    return [
        (match.group(0).replace(" ", ""), line)
        for line in split_lines(text)
        for match in _COST_RE.finditer(line)
    ]
...
def guess_title(text: str) -> str:
    ...
    candidates = [
        ...
        if len(line) > 8 and not line.lower().startswith(skip_prefixes)
    ]
    title_keywords = [
        "trip",
        "notice",
    ...
    return (
        short[0][:90]
        if short
        else (candidates[0][:90] if candidates else "Untitled document")
    )
